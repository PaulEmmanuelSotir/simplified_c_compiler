...
namespace SyntaxModel {
    ArrayConstant<Type::PrimitiveType::CHAR>* make_array_const_from_string(const antlr4::misc::Interval& source_interval, antlr4::tree::TerminalNode* str_token)
    {
        ...
        std::list<const Constant<Type::PrimitiveType::CHAR>*> values(str.length());
        for (const auto& c : str)
            values.push_back(new Constant<Type::PrimitiveType::CHAR>(source_interval, c));

        return new ArrayConstant<Type::PrimitiveType::CHAR>(source_interval, values);
    }
}
